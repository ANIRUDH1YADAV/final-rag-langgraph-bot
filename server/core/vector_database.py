...
from config.settings import GOOGLE_API_KEY, VECTORSTORE_DIRECTORY, MODEL_OPTIONS,HF_TOKEN
...
async def upsert_vectorstore_from_pdfs(
    ...
    logger.debug(
        f"Upserting vectorstore for {model_provider}"
    )

    file_paths = await save_uploaded_file(
        uploaded_files
    )
    ...
    persist_path = VECTORSTORE_DIRECTORY[
        model_provider
    ]
    ...
    logger.debug(
        f"Created fresh vectorstore with "
        f"{len(chunks)} chunks."
    )
